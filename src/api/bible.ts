...
export class FetchError extends Error { // knowledge of custom error class for handling fetch errors with optional HTTP status code
  status?: number;
...
export async function fetchIndex(): Promise<BookIndex> { // Run function to fetch book index data from JSON, returns bookindex
  return fetchJson<BookIndex>(`${DATA_BASE}/index.json`);
}

export async function fetchChapter(bookId: number, chapter: number): Promise<ChapterData> { // Run function to fetch key data from JSON file. 1. know bookid and chapter number, 2. DATA_BASE const —> base URL resolved from importMeta.d.ts 3. Chapter JSON completed
